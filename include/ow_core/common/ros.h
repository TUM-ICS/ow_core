 ...
 */
inline bool load(const std::string& name, bool& val)
{
  if(!has(name))
  {
    return false;
 ...
 */
inline bool load(const std::string& name, int& val)
{
  if(!has(name))
  {
    return false;
 ...
 */
inline bool load(const std::string& name, size_t& val)
{
  int val_;
  if(!load(name, val_))
  {
  ...
  if(val_ < 0)
  {
    ROS_ERROR("load: could not load '%s' as unsigned int.", name.c_str());
    return false;
  }
  val = val_;
  return true;
 ...
 */
inline bool load(const std::string& name, ow::Scalar& val)
{
  if(!has(name))
  {
    return false;
 ...
 */
inline bool load(const std::string& name, std::string& val)
{
  if(!has(name))
  {
    return false;
