 ...
 *
 * Collect and store ros parameters from ros parameters server.
 */
class Parameter
{
public:
...
public:
  Parameter(const std::string& ns = "", bool verbose=false) : 
    ns_(prepareNs(ns)),
    is_loaded_(false),
    verbose_(verbose)
  {
  ...
  Parameter(const ros::NodeHandle& nh, const std::string& sub_ns = "", bool verbose=false) : 
  ...
  bool load()
  {
    ...
    if(verbose_)
    {
      ROS_WARN("Parameters::load(): namespace=%s\n%s:", ns_.c_str(), toString().c_str());
    }
  ...
  bool get(const std::string& name, T& val) const
  {
    if(!is_loaded_)
    {
      ROS_ERROR("Parameter get: Load function was not called for key '%s'.", name.c_str());
      return false;
    }
