...
namespace ow
{
  ...
  /**
   * @brief compute the transformation matrix for omega to quaternion rates.
   * 
   * @note omega is represented in the world frame.
   * Rates: dQ/dt = 0.5*QFixed*omega
   * 
   * @tparam Derived 
   * @param omega 
   * @return Matrix4 
  ...
  Matrix4 QFixed(const ow_core::QuaternionBase<Derived> &Q)
  {
   ...
   * @brief compute the transformation matrix for omega to quaternion rates.
   * 
   * @note omega is represented in the body frame
   * Rates: dQ/dt = 0.5*QBody*omega
   * 
   * @tparam Derived 
   * @param omega 
   * @return Matrix4 
   */
  template <typename Derived>
  Matrix4 QBody(const ow_core::QuaternionBase<Derived> &Q)
  {
    const Derived &Q_ = Q.derived();
    return (Matrix4() << 
            Q_.w(), -Q_.x(), -Q_.y(), -Q_.z(),
  ...
  /**
   * @brief compute the transformation matrix for quaternion rates to omega
   * 
   * @note omega is represented in world frame
   * omega: omega = 2*HFixed*dQ/dt
   * 
   * @tparam Derived 
   * @param Q 
   ...
   */
  template <typename Derived>
  Eigen::Matrix<Scalar, 3, 4> HFixed(const ow_core::QuaternionBase<Derived> &Q)
  {
    const Derived &Q_ = Q.derived();
    return (Eigen::Matrix<Scalar, 3, 4>() << 
            Q_(3), -Q_(2), Q_(1), -Q_(0),
            Q_(2), Q_(3), -Q_(0), -Q_(1),
            -Q_(1), Q_(0), Q_(3), -Q_(2)).finished();
  }
  ...
  */
      ...
      const ow_core::QuaternionBase<Derived2> &Q)
  {
    ...
    // Relative quaternion shortest distance
  ...
  */
  ...
  {
    typedef ow::Scalar Scalar;
    Eigen::Matrix<Scalar, 3, 4> H = HFixed(Q);
  ...
  */
      ...
      ow::Scalar s)
  {
    ...
    Qs.coeffs() = scale_0 * Qi.derived().coeffs() + scale_1 * Qf.derived().coeffs();
    return Qs;
  }
  ...
  */
      ...
      ow::Scalar s)
  {
    ...
    QsP.coeffs() = scale_0 * Qi.derived().coeffs() + scale_1 * Qf.derived().coeffs();
    return QsP;
  }
  ...
  */
      ...
      ow::Scalar s)
  {
    ...
    QsPP.coeffs() = scale_0 * Qi.derived().coeffs() + scale_1 * Qf.derived().coeffs();
    return QsPP;
  }
